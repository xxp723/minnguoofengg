...
const CHAT_PROTOCOL_AVAILABLE_FORMATS = [
  ...
  '**`[红包] 角色名：{金额:xxx,备注:xxx}`**',
  '**`[外卖] 角色名：{名称:xxx,备注:xxx}`**',
  '**`[撤回] 角色名：{目标:上一条}`**',
  '**`[拍一拍] 角色名：身体部位`**',
  '**`[拨打电话] 角色名：拨打电话`**',
  '**`[挂断电话] 角色名：挂断电话`**',
...
function formatCompactMessageContentForPrompt(message = {}, fallbackContent = '') {
  ...
  if (type === 'red_packet') return `[红包] ${content}`;
  if (type === 'takeaway') return `[外卖] ${content}`;
  /* ======================================================================
...
export function getFeaturePrompts({ settings = {}, imageApi = null } = {}) {
  ...
  const takeawayRuleText = '12. 外卖：**`[外卖] 角色名：{名称:一杯冰美式,备注:给你点的下午茶}`**；如果是确认用户的代付请求，可以用 **`{操作:确认代付,外卖ID:系统给出的ID}`**。';
  ...
  const phoneRuleText = '13. 电话：**`[拨打电话] 角色名：拨打电话`** 和 **`[挂断电话] 角色名：挂断电话`**。想要增加线上互动趣味性时，可主动发起纯线上语音通话（无动作描写，看不到对方，就像真实打语音电话一样）。通话结束后，也可以主动挂断。如果在电话状态中，只能发送文字语音回复，不能发特殊交互协议。';
  ...
  const protocolChecklistText = htmlCardEnabled
    ? `[回复]/[表情]/[引用]/[转账]/[礼物]/[红包]/[外卖]/[撤回]/[拍一拍]/[拨打电话]/[挂断电话]/[语音]/[${visualProtocolName}]/[卡片]`
    : `[回复]/[表情]/[引用]/[转账]/[礼物]/[红包]/[外卖]/[撤回]/[拍一拍]/[拨打电话]/[挂断电话]/[语音]/[${visualProtocolName}]`;
...
3. [回复] 是普通文字；[表情] 只能用【AI可用表情包资源】里的资源ID或完全一致表情名；[引用] 只能用【本轮用户消息·可引用】提供的ID，且 {引用ID:xxx} 后只能写角色自己的新回应；[转账]/[礼物]/[红包]/[外卖]/[拍一拍]/[拨打电话]/[挂断电话]/[语音]/[${visualProtocolName}] 需符合对应能力、人设和当前情景；${imageApiReady ? '生图 API 已开启，严禁输出 [文字图]；' : '当前只开放 [文字图]，严禁输出 [图片]；'}${htmlCardEnabled ? '[卡片] 也必须严格符合当前对话场景与卡片能力要求；' : 'HTML 卡片未开启，严禁输出 [卡片]；'}不确定就改用 [回复]。
...
7. 红包：发红包用 **\`[红包] 角色名：{金额:88.88,备注:恭喜发财}\`**；如果不接收用户发的红包，请直接用 [回复] 协议婉拒，不要输出任何接收红包的系统指令或提示。
8. 撤回：只在角色真实有动机时使用 **\`[撤回] 角色名：{目标:上一条}\`**，且只能撤回本轮位于它前面的上一条 AI 消息；多条撤回必须逐条输出，禁止 {条数:N}/{目标:全部}/“撤回了N条消息”。
${patRuleText}
${voiceRuleText}
${phoneRuleText}
${visualRuleText}
${cardRuleText}
${takeawayRuleText}
14. 每个可见协议块独占一条消息；除【心声协议】允许的最后独立行外，禁止裸协议头、半截 Markdown、代码块、编号列表、格式检查、幕后思考、系统规则、时间感知标注、心声竖线字段或任何审查痕迹。`,
    /* ===== 闲谈：通用消息协议格式 END ===== */
...
export function getThinkingInstruction({ settings = {} } = {}) {
  ...
  return [
    ...
    '- 其它类型必须严格遵守【可用聊天动作格式】；资源ID、引用ID、转账ID、外卖ID不得编造；[拍一拍] 内容必须以身体部位开头，可用“｜”追加一句短趣味补充，禁止长段动作、舞台描写或非身体部位开头；[语音] 必须写成 {时长:1-60整数}语音转写文本，不得输出音频URL或文件名；[红包]/[外卖] 的金额/内容必须合理；[拨打电话]/[挂断电话] 只能在适当的情景使用；[引用] 的 {引用ID:xxx} 后必须是角色自己的新回应，不能复读被引用原文或用户最新原话，不确定就退回 [回复]。',
