...
export function getFeaturePrompts({ settings = {}, imageApi = null } = {}) {
  ...
  /* ======================================================================
     ...
     2. 内容只能是身体部位，最终显示为“角色名拍了拍你的身体部位”。
     3. 仅调整本轮系统提示词，不新增持久化存储，不使用 localStorage/sessionStorage。
     ====================================================================== */
  const patRuleText = '8. 拍一拍：**`[拍一拍] 角色名：身体部位`**，像 QQ/微信戳一戳；只在想轻轻提醒用户回复、安慰、撒娇、逗用户或贴合当前情绪时偶尔使用；内容只能写一个身体部位，如“肩膀/脑袋/手背/后背”，禁止写整句话、动作描写、非身体部位、隐私部位、暴力/色情部位或任何解释；不能掉格式，前端只显示系统小字“角色名拍了拍你的身体部位”。';
...
export function getThinkingInstruction({ settings = {} } = {}) {
  ...
  return [
    ...
    '- 其它类型必须严格遵守【可用聊天动作格式】；资源ID、引用ID、转账ID不得编造；[拍一拍] 内容只能是身体部位，禁止写整句动作或非身体部位；[语音] 必须写成 {时长:1-60整数}语音转写文本，不得输出音频URL或文件名；[引用] 的 {引用ID:xxx} 后必须是角色自己的新回应，不能复读被引用原文或用户最新原话，不确定就退回 [回复]。',
