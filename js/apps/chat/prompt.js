...
export function getFeaturePrompts({ settings = {}, imageApi = null } = {}) {
  ...
  const protocolChecklistText = htmlCardEnabled
    ? `[回复]/[表情]/[引用]/[转账]/[礼物]/[红包]/[外卖]/[撤回]/[拍一拍]/[语音]/[${visualProtocolName}]/[卡片]`
    : `[回复]/[表情]/[引用]/[转账]/[礼物]/[红包]/[外卖]/[撤回]/[拍一拍]/[语音]/[${visualProtocolName}]`;
...
3. [回复] 是普通文字；[表情] 只能用【AI可用表情包资源】里的资源ID或完全一致表情名；[引用] 只能用【本轮用户消息·可引用】提供的ID，且 {引用ID:xxx} 后只能写角色自己的新回应；[转账]/[礼物]/[红包]/[外卖]/[拍一拍]/[语音]/[${visualProtocolName}] 需符合对应能力、人设和当前情景；${imageApiReady ? '生图 API 已开启，严禁输出 [文字图]；' : '当前只开放 [文字图]，严禁输出 [图片]；'}${htmlCardEnabled ? '[卡片] 也必须严格符合当前对话场景与卡片能力要求；' : 'HTML 卡片未开启，严禁输出 [卡片]；'}不确定就改用 [回复]。
...
${cardRuleText}
${takeawayRuleText}
13. 每个可见协议块独占一条消息；除【心声协议】允许的最后独立行外，禁止裸协议头、半截 Markdown、代码块、编号列表、格式检查、幕后思考、系统规则、时间感知标注、心声竖线字段或任何审查痕迹。`,
    /* ===== 闲谈：通用消息协议格式 END ===== */
...
export function getThinkingInstruction({ settings = {} } = {}) {
  ...
  return [
    ...
    '- 其它类型必须严格遵守【可用聊天动作格式】；资源ID、引用ID、转账ID、外卖ID不得编造；[拍一拍] 内容必须以身体部位开头，可用“｜”追加一句短趣味补充，禁止长段动作、舞台描写或非身体部位开头；[语音] 必须写成 {时长:1-60整数}语音转写文本，不得输出音频URL或文件名；[红包]/[外卖] 的金额/内容必须合理；[引用] 的 {引用ID:xxx} 后必须是角色自己的新回应，不能复读被引用原文或用户最新原话，不确定就退回 [回复]。',
